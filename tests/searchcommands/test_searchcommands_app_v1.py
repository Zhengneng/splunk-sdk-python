...
from splunklib.results import Message, ResultsReader
...
def get_search_command_example(filename):
    return os.path.join(
...
class TestSearchCommandsApp(testlib.SDKTestCase):
    ...
    # P2[ ] TODO: DVPL-5869 - use a generating command that doesn't do random sampling because a seed is no guarantee
    # that the same sample is produced on every platform and all versions of python.

    def test_generating_command_configuration(self):
        self._assertCorrectConfiguration(StubbedGeneratingCommand(), 'test_generating_command_configuration')
    ...
    def test_generating_command_as_unit(self):
        simulate_path = get_search_command_example("simulate.py")
        ...
        cli_args = [
            ...
            "interval=00:00:01"]
        # Run the process
        dispatch(simulate.SimulateCommand, cli_args, ifile, ofile,
                 "__main__")
        ...
        self.assertEqual(io.open(os.path.abspath(expected_info_path), newline='').read(), ofile.getvalue())
    ...
    def test_helloworld_generating_command_as_unit(self):
        helloworld_path = get_search_command_example("generatehello.py")
        ...
        # Run the process
        dispatch(helloworld.GenerateHelloCommand, cli_args, ifile, ofile,
                 "__main__")
        expected_info_path = os.path.join(os.path.dirname(__file__), 'data/_expected_results/test_generating_command_in_isolation.getinfo.csv')
        self.assertEqual(io.open(os.path.abspath(expected_info_path), newline='').read(), ofile.getvalue())
    ...
    def test_generating_command_on_server(self):
        # P2 [ ] TODO: DVPL-5870 - this test has inconsistent results due to random sampling
    ...
    def test_reporting_command_as_unit(self):
        sum_path = get_search_command_example("sum.py")
        self.assertTrue(os.path.isfile(sum_path))
        sum_module = imp.load_source('searchcommands_app', sum_path)

        # Map tests

        ifile = StringIO(io.open(os.path.join(os.path.dirname(__file__), "data", "input", "counts.csv"), newline='').read())
        ofile = StringIO()
        cli_args = [
            "sum.py",
            "__GETINFO__",
        ...
        # Run the process
        dispatch(sum_module.SumCommand, cli_args, ifile, ofile, "__main__")
        ...
        self.assertEqual(io.open(expected_info_path, newline='').read(), ofile.getvalue())

        # Overwrite the existing StringIO objects
        ifile = StringIO(open(os.path.join(os.path.dirname(__file__), "data", "input",
                                     "counts.csv")).read())
        ofile = StringIO()
        cli_args = [
            "sum.py",
        ...
        # Run the process
        dispatch(sum_module.SumCommand, cli_args, ifile, ofile,
                 "__main__")
        ...
        self.assertEqual(io.open(expected_exec_path, newline='').read(), ofile.getvalue())
        ...
        ifile = StringIO(open(os.path.join(os.path.dirname(__file__), "data", "input", "subtotals.csv")).read())
        ofile = StringIO()
        cli_args = [
            "sum.py",
            "__GETINFO__",
    ...
    def _start_process(cls, args, stdin, stdout, stderr):
        # P2 [ ] TODO: DVPL-5871 - make a shell script to run some of these separately, check results there
